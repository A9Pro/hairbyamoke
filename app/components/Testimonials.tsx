...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="section" style={{ background:"var(--black)", position:"relative", overflow:"hidden" }}>
      ...
      <div className="df" style={{ position:"absolute", top:"20px", left:"8px",
        fontSize:"16rem", lineHeight:1, fontWeight:300,
        color:"rgba(201,168,76,0.05)", pointerEvents:"none", userSelect:"none", zIndex:1 }}>"</div>
      <div style={{ position:"absolute", inset:0,
      ...
      <div className="container" style={{ position:"relative", zIndex:2 }}>
        <div style={{ maxWidth:"760px", margin:"0 auto" }}>
          ...
          <div className="card testi-card">
            {/* Avatar — plain img with explicit dimensions, no next/image */}
            <div style={{ display:"flex", justifyContent:"center", marginBottom:"18px" }}>
              <div style={{ width:"70px", height:"70px", borderRadius:"50%",
                overflow:"hidden", border:"2px solid rgba(201,168,76,0.38)",
                backgroundColor:"var(--dark-3)", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"18px", marginTop:"26px" }}>
            <button onClick={() => setCur(c => (c - 1 + ts.length) % ts.length)}
              style={{ width:"36px", height:"36px", border:"1px solid rgba(201,168,76,0.3)",
                background:"transparent", color:"var(--gold)", cursor:"pointer",
                fontSize:"20px", display:"flex", alignItems:"center", justifyContent:"center" }}>‹</button>
            <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
              {ts.map((_,i) => (
                <div key={i} onClick={() => setCur(i)} style={{ height:"2px",
                  width:i===cur?"22px":"6px",
                  background:i===cur?"var(--gold)":"rgba(201,168,76,0.2)",
                  borderRadius:"1px", cursor:"pointer", transition:"all 0.3s" }} />
              ))}
            </div>
            <button onClick={() => setCur(c => (c + 1) % ts.length)}
              style={{ width:"36px", height:"36px", border:"1px solid rgba(201,168,76,0.3)",
                background:"transparent", color:"var(--gold)", cursor:"pointer",
                fontSize:"20px", display:"flex", alignItems:"center", justifyContent:"center" }}>›</button>
          </div>
